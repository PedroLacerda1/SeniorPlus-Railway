...
1. `mvn -pl backend -am org.owasp:dependency-check-maven:check -DskipTests=true` keeps the backend dependencies patched. We recently
   bumped `mysql-connector-j` to `8.4.0` and added an explicit `commons-lang3` dependency at `3.19.0` to satisfy the latest CVE alerts.
